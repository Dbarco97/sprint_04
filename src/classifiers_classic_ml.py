...
def visualize_embeddings(X_train, X_test, y_train, y_test, plot_type='2D', method='PCA'): 
    ...
    if plot_type == '3D':
        if method == 'PCA':
            ...
            red = TSNE(n_components=3, perplexity=perplexity).fit(X_train)
        ...
        if method == 'PCA':
            ...
            red = TSNE(n_components=2, perplexity=perplexity).fit(X_train)
            # TODO: Use the model to train and transform the test data
            reduced_embeddings = red.transform(X_test)
        else:
            raise ValueError("Invalid method. Please choose either 'PCA' or 't-SNE'.")
...
def train_and_evaluate_model(X_train, X_test, y_train, y_test, models=None, test=True):
    ...
    for name, model in models:
        ...
        if test:
            ...
            print(f'Accuracy, Precision, Recall, F1: {accuracy:.2f}, {precision:.2f}, {recall:.2f},{f1:.2f}')
